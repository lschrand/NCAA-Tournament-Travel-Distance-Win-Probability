...
.chart-guide p { margin: 0; color: #94a3b8 !important; font-size: 0.85rem; line-height: 1.5; }
.chart-guide strong { color: #e2e8f0 !important; }
.how-to-read {
...
.how-to-read p { margin: 0; color: #94a3b8 !important; font-size: 0.85rem; line-height: 1.5; }
.how-to-read strong { color: #e2e8f0 !important; }
/* Header photo styling */
[data-testid="stImage"] img {
    border-radius: 50%; border: 3px solid #f97316;
...
def load_and_process_data():
    ...
    shooting = pd.read_csv(os.path.join(data_dir, 'Shooting Splits.csv'))
    enh = td.copy()
    ...
    return dict(team_data=td, games_df=games_df, y_all=ya,
        ...
        kenpom=kp, conf_stats=cs, shooting=shooting, features_tr=f_tr, sc4=sc4, X4s=X4, lr4=lr4, cv4=cv4, y4=y4,
...
try:
    ...
    st.error(f"**Error loading data:** {e}\n\nMake sure `Tournament Locations.csv`, "
             ...
             "`Conference Stats.csv`, and `Shooting Splits.csv` are in the same directory.")
    st.stop()
...
with h_left:
    ...
    st.markdown(
        ...
        f"{min(all_years)}–{max(all_years)} · 5 ML models`"
    )
    st.markdown(
        '<p style="color: #94a3b8; font-size: 0.85rem; margin-top: 4px;">'
        'Created by <strong style="color: #f97316;">Levi Schrand</strong> · '
        'Purdue University · Team, Conference & Distance Analytics</p>',
        unsafe_allow_html=True
    )
with h_right:
    try:
        import os as _os
        _photo_path = _os.path.join(_os.path.dirname(__file__), 'teams_pic.jpg') if '__file__' in dir() else 'teams_pic.jpg'
        if _os.path.exists(_photo_path):
            st.image(_photo_path, width=120)
        elif _os.path.exists('teams_pic.jpg'):
            st.image('teams_pic.jpg', width=120)
    except Exception:
        pass
st.markdown("---")

# ── Master tabs ───────────────────────────────────────────────────────────
master_predict, master_travel, master_shooting, master_conf, master_models = st.tabs([
    "🎯 Predictions & Matchups", "📊 Travel & Distance",
    "🏀 Shot Profile DNA", "🏛️ Conferences", "🤖 Models"])
...
with master_predict:
    ...
    with tab_pred:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'This tool estimates a team\'s <strong>probability of winning</strong> a tournament game '
            'based on how far they travel and their seed. Adjust the sliders below to explore '
            '"what if" scenarios — for example, what happens to a 4-seed\'s chances when they '
            'have to travel 1,500 miles vs. 200 miles?'
        ...
        st.markdown('<div class="how-to-read"><p>'
            '📊 <strong>Reading the charts below:</strong> '
            'The <strong>gauge</strong> (left) shows the predicted win probability — green is favorable, red is not. '
            'The <strong>win curve</strong> (center) shows how probability changes as distance increases for this seed — '
            'the cyan dot marks your selected distance. '
            'The <strong>histogram</strong> (right) compares the travel distances of historical winners vs losers '
            'for this seed/round combo.'
            '</p></div>', unsafe_allow_html=True)
    ...
    with tab_enh:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'This predictor goes beyond seed and distance — it incorporates '
            '<strong>TeamRankings power ratings</strong>, <strong>KenPom efficiency margins</strong>, '
            '<strong>strength of schedule</strong>, and <strong>wins against top-25 teams</strong>. '
            'Pick two real tournament teams to see how three increasingly sophisticated models '
            'rate the matchup, or enter custom stats to explore hypotheticals.'
            '</p></div>', unsafe_allow_html=True)
        ...
        if mode == "Pick Tournament Teams":
            ...
            if ta==tb: st.warning("Pick two different teams.")
                ...
                st.markdown('<div class="how-to-read"><p>'
                    '📊 <strong>How to read:</strong> Each bar shows the relative win probability '
                    'from a different model. <strong>Simple</strong> uses only distance + seed. '
                    '<strong>Enhanced</strong> adds TeamRankings power rating and SOS. '
                    '<strong>Full</strong> adds KenPom efficiency and improvement from preseason. '
                    'Higher AUC = more accurate model. When all models agree, you can be more confident.'
                    '</p></div>', unsafe_allow_html=True)
    ...
    with tab_h2h:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'Pit two hypothetical teams against each other by setting their <strong>seed</strong> '
            'and <strong>travel distance</strong>. The left chart shows the relative win probability, '
            'while the right chart reveals how <strong>sensitive</strong> the prediction is to Team A\'s '
            'distance — useful for understanding how much home-court proximity actually matters in a matchup.'
            '</p></div>', unsafe_allow_html=True)
        ca, cb = st.columns(2)
    ...
    with tab_team:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'Search for any team to see their complete <strong>tournament travel profile</strong>. '
            'The scatter plot shows each game color-coded by outcome (green = win, red = loss) '
            'plotted against distance traveled. The bar chart compares their win rate when playing '
            '<strong>close to home vs. far away</strong>, and the histogram shows their typical travel distances.'
            '</p></div>', unsafe_allow_html=True)
...
with master_travel:
    ...
    with tab_exp:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'Does traveling farther hurt your chances? This tab buckets teams by '
            '<strong>how far they traveled</strong> and shows the win rate for each group. '
            'The bar chart (left) reveals whether close teams win more often, while the '
            'scatter plot (right) shows every individual game outcome with a logistic regression '
            'trend line. Filter by round using the dropdown — the effect may vary by tournament stage. '
            'Year and seed filters come from the <strong>sidebar</strong>.'
            '</p></div>', unsafe_allow_html=True)
        ...
        if len(df)<20: st.warning(f"Not enough data (n={len(df)}). Broaden sidebar filters.")
            ...
            st.markdown('<div class="how-to-read"><p>'
                ...
                '<strong>Correlation (r)</strong> measures the relationship strength — negative means '
                'more distance = fewer wins. '
                '<strong>p-value</strong> below 0.05 means the relationship is statistically significant. '
                '<strong>Losers +</strong> shows how many extra miles losing teams traveled on average.'
                '</p></div>', unsafe_allow_html=True)
    ...
    with tab_rnd:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'The distance advantage may not be equal across all rounds. Early rounds are played at '
            'regional sites (giving nearby teams an edge), while the Final Four is at a neutral site. '
            'This tab compares <strong>close vs. far</strong> win rates for each round, '
            'shows the <strong>correlation strength</strong> (faded = not statistically significant), '
            'and displays sample sizes so you can judge data reliability. Uses sidebar filters.'
            '</p></div>', unsafe_allow_html=True)
    ...
    with tab_seed:
        ...
        st.markdown('<div class="chart-guide"><p>'
            'Do higher seeds benefit more from playing close to home? This tab splits each seed\'s '
            'games into <strong>"close" (below median distance)</strong> and <strong>"far" (above median)</strong> '
            'and compares win rates. The right chart shows the <strong>advantage in percentage points</strong> — '
            'positive means close teams win more. Useful for spotting which seeds are most '
            'affected by travel.'
            '</p></div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════
#  MASTER 3: SHOT PROFILE DNA
# ═══════════════════════════════════════════════════════════════════════════
with master_shooting:
    st.markdown('<div class="landing-card"><h4>🏀 Shot Profile DNA</h4>'
        '<p>Every tournament team has a unique shooting fingerprint — how they distribute shots '
        'across dunks, close twos, mid-range, and threes, and how efficiently they score and defend '
        'each zone. Explore team profiles, compare matchups, and see what a champion\'s '
        'shooting DNA looks like.</p></div>', unsafe_allow_html=True)

    tab_dna, tab_champ = st.tabs(["🔬 Team Shot DNA", "🏆 Champion Archetype"])

    # Build shooting + outcome merged data for reuse
    _ss_merged = team_data.merge(
        shooting[['YEAR','TEAM NO','TEAM','CONF',
    ...
    # Deepest run per team per year
    _team_runs = _ss_merged.groupby(['YEAR','TEAM NO']).agg(
        deepest=('ROUND','min'), games_won=('WON','sum'), seed=('SEED','first'),
        team=('TEAM','first')
    ).reset_index()
    _team_runs = _team_runs.merge(
        shooting[['YEAR','TEAM NO','CONF',
                  'DUNKS FG%','DUNKS SHARE','DUNKS FG%D','DUNKS D SHARE',
                  'CLOSE TWOS FG%','CLOSE TWOS SHARE','CLOSE TWOS FG%D','CLOSE TWOS D SHARE',
                  'FARTHER TWOS FG%','FARTHER TWOS SHARE','FARTHER TWOS FG%D','FARTHER TWOS D SHARE',
                  'THREES FG%','THREES SHARE','THREES FG%D','THREES D SHARE']],
        on=['YEAR','TEAM NO'], how='inner'
    )
    _team_runs['run'] = _team_runs['deepest'].map(
        {1:'Champion',2:'Runner-Up',4:'Final Four',8:'Elite 8',
         16:'Sweet 16',32:'Round of 32',64:'Round of 64'})
    _shot_zones = ['Dunks','Close Twos','Mid-Range','Threes']
    _share_cols = ['DUNKS SHARE','CLOSE TWOS SHARE','FARTHER TWOS SHARE','THREES SHARE']
    _fg_cols = ['DUNKS FG%','CLOSE TWOS FG%','FARTHER TWOS FG%','THREES FG%']
    _fg_d_cols = ['DUNKS FG%D','CLOSE TWOS FG%D','FARTHER TWOS FG%D','THREES FG%D']

    # ── Tab 1: Team Shot DNA ─────────────────────────────────────────────
    with tab_dna:
        st.subheader("Team Shot DNA")
        st.markdown('<div class="chart-guide"><p>'
            'Every team distributes their shots differently — some live at the rim with dunks and '
            'close twos, others rely on three-point shooting. This tool visualizes a team\'s '
            '<strong>offensive shot distribution</strong>, <strong>shooting accuracy by zone</strong>, '
            'and <strong>defensive effectiveness</strong> at each zone. Compare two teams side by side '
            'to spot mismatches.</p></div>', unsafe_allow_html=True)

        c1, c2, c3 = st.columns([1, 2, 2])
        with c1:
            dna_year = st.selectbox("Season", sorted(shooting['YEAR'].unique(), reverse=True), key="dna_yr")
        yr_teams = sorted(shooting[shooting['YEAR'] == dna_year]['TEAM'].unique())
        default_a = g_fav if g_fav != "—" and g_fav in yr_teams else yr_teams[0]
        with c2:
            dna_team_a = st.selectbox("Team A", yr_teams,
                index=yr_teams.index(default_a) if default_a in yr_teams else 0, key="dna_a")
        with c3:
            dna_team_b = st.selectbox("Team B (compare)", ["— None —"] + yr_teams, key="dna_b")

        row_a = shooting[(shooting['YEAR'] == dna_year) & (shooting['TEAM'] == dna_team_a)]
        if len(row_a) == 0:
            st.warning(f"No shooting data for {dna_team_a} in {dna_year}.")
        else:
            row_a = row_a.iloc[0]
            comparing = dna_team_b != "— None —"
            row_b = None
            if comparing:
                rb = shooting[(shooting['YEAR'] == dna_year) & (shooting['TEAM'] == dna_team_b)]
                if len(rb): row_b = rb.iloc[0]
                else: comparing = False

            # ── Shot Distribution (stacked bar / pie comparison) ─────
            st.markdown("#### Shot Distribution")
            st.markdown('<div class="how-to-read"><p>'
                '📊 <strong>How to read:</strong> Each bar shows what percentage of a team\'s shots '
                'come from each zone. Teams with high <strong>Dunks + Close Twos</strong> share play '
                'an inside-out style. Teams with high <strong>Threes</strong> share rely on perimeter shooting. '
                'Tournament champions since 2010 average ~10% dunks and ~36% threes.</p></div>',
                unsafe_allow_html=True)

            shares_a = [row_a['DUNKS SHARE'], row_a['CLOSE TWOS SHARE'],
                        row_a['FARTHER TWOS SHARE'], row_a['THREES SHARE']]
            zone_colors = [GREEN, CYAN, ORANGE, PURPLE]

            if comparing:
                shares_b = [row_b['DUNKS SHARE'], row_b['CLOSE TWOS SHARE'],
                            row_b['FARTHER TWOS SHARE'], row_b['THREES SHARE']]
                fig = make_subplots(1, 2, subplot_titles=[dna_team_a, dna_team_b],
                                    specs=[[{"type":"pie"},{"type":"pie"}]])
                fig.add_trace(go.Pie(labels=_shot_zones, values=shares_a,
                    marker=dict(colors=zone_colors), textinfo='label+percent',
                    textfont=dict(size=12), hole=0.35), row=1, col=1)
                fig.add_trace(go.Pie(labels=_shot_zones, values=shares_b,
                    marker=dict(colors=zone_colors), textinfo='label+percent',
                    textfont=dict(size=12), hole=0.35), row=1, col=2)
            else:
                fig = go.Figure(go.Pie(labels=_shot_zones, values=shares_a,
                    marker=dict(colors=zone_colors), textinfo='label+percent+value',
                    textfont=dict(size=13), hole=0.4,
                    hovertemplate='%{label}: %{value:.1f}% of shots'))

            fig.update_layout(**plotly_layout(height=350, showlegend=False))
            st.plotly_chart(fig, use_container_width=True)

            # ── Shooting Accuracy by Zone ────────────────────────────
            st.markdown("#### Shooting Accuracy by Zone")
            st.markdown('<div class="how-to-read"><p>'
                '📊 <strong>How to read:</strong> This shows <strong>FG%</strong> for each shot type. '
                'Dunks are near 90% for everyone — the real differentiators are '
                '<strong>close twos</strong> (elite teams shoot 65%+) and <strong>threes</strong> '
                '(36%+ is strong). Green = offense, Red = defense (lower is better for defense).'
                '</p></div>', unsafe_allow_html=True)

            fg_a = [row_a[c] for c in _fg_cols]
            fgd_a = [row_a[c] for c in _fg_d_cols]

            if comparing:
                fg_b = [row_b[c] for c in _fg_cols]
                fgd_b = [row_b[c] for c in _fg_d_cols]
                fig = make_subplots(1, 2, subplot_titles=[
                    f"{dna_team_a} — Accuracy by Zone", f"{dna_team_b} — Accuracy by Zone"])
                for col_idx, (fg, fgd, name) in enumerate([(fg_a, fgd_a, dna_team_a),
                                                            (fg_b, fgd_b, dna_team_b)], 1):
                    fig.add_trace(go.Bar(x=_shot_zones, y=fg, name='Offense' if col_idx==1 else None,
                        marker_color=GREEN, showlegend=(col_idx==1),
                        text=[f"{v:.1f}%" for v in fg], textposition='outside'), row=1, col=col_idx)
                    fig.add_trace(go.Bar(x=_shot_zones, y=fgd, name='Defense' if col_idx==1 else None,
                        marker_color=RED, showlegend=(col_idx==1),
                        text=[f"{v:.1f}%" for v in fgd], textposition='outside'), row=1, col=col_idx)
            else:
                fig = go.Figure()
                fig.add_trace(go.Bar(x=_shot_zones, y=fg_a, name='Offense (FG%)',
                    marker_color=GREEN, text=[f"{v:.1f}%" for v in fg_a], textposition='outside'))
                fig.add_trace(go.Bar(x=_shot_zones, y=fgd_a, name='Defense (Opp FG%)',
                    marker_color=RED, text=[f"{v:.1f}%" for v in fgd_a], textposition='outside'))

            fig.update_layout(**plotly_layout(height=400, barmode='group'))
            fig.update_yaxes(title_text="FG %")
            st.plotly_chart(fig, use_container_width=True)

            # ── Radar Chart: Offensive DNA Fingerprint ───────────────
            st.markdown("#### Offensive DNA Fingerprint")
            st.markdown('<div class="how-to-read"><p>'
                '📊 <strong>How to read:</strong> The radar chart normalizes each metric against all '
                'tournament teams that year (0–100 percentile). A perfectly balanced team fills the '
                'radar evenly. Spikes show strengths; dips show weaknesses. '
                'Hover for exact values.</p></div>', unsafe_allow_html=True)

            # Compute percentiles within that year
            yr_data = shooting[shooting['YEAR'] == dna_year]
            radar_cols = ['DUNKS SHARE','CLOSE TWOS FG%','FARTHER TWOS FG%','THREES FG%',
                          'THREES SHARE','DUNKS FG%D','THREES FG%D']
            radar_labels = ['Dunks Share','Close 2s Accuracy','Mid-Range Accuracy',
                            '3PT Accuracy','3PT Volume','Rim Protection\n(lower=better)',
                            '3PT Defense\n(lower=better)']

            def pctile(val, series, invert=False):
            ...
            for col, label in zip(radar_cols, radar_labels):
                inv = 'Defense' in label or 'lower' in label
                p = pctile(row_a[col], yr_data[col], invert=inv)
                vals_a.append(p)
                hover_a.append(f"{label}: {row_a[col]:.1f} ({p:.0f}th %ile)")
            vals_a.append(vals_a[0]); hover_a.append(hover_a[0])
            labels_closed = radar_labels + [radar_labels[0]]
            fig.add_trace(go.Scatterpolar(r=vals_a, theta=labels_closed, fill='toself',
                name=dna_team_a, line=dict(color=GREEN, width=2),
                fillcolor='rgba(34,197,94,0.15)', hovertext=hover_a, hoverinfo='text'))
            ...
            if comparing:
                ...
                for col, label in zip(radar_cols, radar_labels):
                    inv = 'Defense' in label or 'lower' in label
                    p = pctile(row_b[col], yr_data[col], invert=inv)
                    vals_b.append(p)
                    hover_b.append(f"{label}: {row_b[col]:.1f} ({p:.0f}th %ile)")
                vals_b.append(vals_b[0]); hover_b.append(hover_b[0])
                fig.add_trace(go.Scatterpolar(r=vals_b, theta=labels_closed, fill='toself',
                    name=dna_team_b, line=dict(color=ORANGE, width=2),
                    fillcolor='rgba(249,115,22,0.15)', hovertext=hover_b, hoverinfo='text'))

            fig.update_layout(**plotly_layout(height=480, showlegend=True),
                polar=dict(bgcolor='rgba(0,0,0,0)',
                    radialaxis=dict(visible=True, range=[0, 100], gridcolor='#1e2530',
            ...
            stats_rows = []
            for zone, sh, fg, fgd, shd in [
                ('Dunks','DUNKS SHARE','DUNKS FG%','DUNKS FG%D','DUNKS D SHARE'),
                ('Close Twos','CLOSE TWOS SHARE','CLOSE TWOS FG%','CLOSE TWOS FG%D','CLOSE TWOS D SHARE'),
                ('Mid-Range','FARTHER TWOS SHARE','FARTHER TWOS FG%','FARTHER TWOS FG%D','FARTHER TWOS D SHARE'),
                ('Threes','THREES SHARE','THREES FG%','THREES FG%D','THREES D SHARE'),
            ]:
                r = {'Zone': zone,
                     f'{dna_team_a} Share': f"{row_a[sh]:.1f}%",
                     f'{dna_team_a} FG%': f"{row_a[fg]:.1f}%",
                     f'{dna_team_a} Def FG%': f"{row_a[fgd]:.1f}%"}
                if comparing:
                    r[f'{dna_team_b} Share'] = f"{row_b[sh]:.1f}%"
                    r[f'{dna_team_b} FG%'] = f"{row_b[fg]:.1f}%"
                    r[f'{dna_team_b} Def FG%'] = f"{row_b[fgd]:.1f}%"
                stats_rows.append(r)
            st.dataframe(pd.DataFrame(stats_rows), use_container_width=True, hide_index=True)

    # ── Tab 2: Champion Archetype ────────────────────────────────────────
    with tab_champ:
        st.subheader("What Does a Champion Look Like?")
        st.markdown('<div class="chart-guide"><p>'
            'Champions aren\'t random — they share a consistent shooting DNA. Since 2010, '
            'title winners average <strong>10% dunk share</strong> (vs 6.3% for first-round exits), '
            '<strong>65.6% close-twos accuracy</strong> (vs 61.3%), and '
            '<strong>31.5% opponent 3PT%</strong> (vs 33.2%). '
            'This tab shows how the "champion archetype" compares to each round of elimination, '
            'and lets you check how any team stacks up against it.</p></div>', unsafe_allow_html=True)

        # ── Shot Profile by Tournament Depth ─────────────────────────
        st.markdown("#### Shot Profile by Tournament Depth")
        st.markdown('<div class="how-to-read"><p>'
            '📊 <strong>How to read:</strong> Each group of bars represents teams whose '
            '<strong>deepest run</strong> was that round. A steady gradient from left to right '
            'means the metric matters for advancing. The champion bar is highlighted.</p></div>',
            unsafe_allow_html=True)

        run_order = ['Round of 64','Round of 32','Sweet 16','Elite 8','Final Four','Runner-Up','Champion']
        depth_metrics = ['DUNKS SHARE','CLOSE TWOS FG%','THREES FG%','THREES FG%D']
        depth_labels = ['Dunk Share %','Close Twos FG%','Three-Point FG%','Opp 3PT% (↓ better)']

        fig = make_subplots(1, 4, subplot_titles=depth_labels)
        for i, (col, label) in enumerate(zip(depth_metrics, depth_labels), 1):
            vals = []
            labels_r = []
            colors = []
            for r in run_order:
                rd = _team_runs[_team_runs['run'] == r]
                if len(rd) >= 3:
                    vals.append(rd[col].mean())
                    labels_r.append(r.replace('Round of ', 'R'))
                    colors.append('#fbbf24' if r == 'Champion' else BLUE)
            fig.add_trace(go.Bar(x=labels_r, y=vals, marker_color=colors, showlegend=False,
                text=[f"{v:.1f}" for v in vals], textposition='outside',
                hovertemplate='%{x}: %{y:.1f}'), row=1, col=i)
        fig.update_layout(**plotly_layout(height=400))
        st.plotly_chart(fig, use_container_width=True)

        # ── Champion Radar vs Your Team ──────────────────────────────
        st.markdown("#### Compare a Team to the Champion Archetype")
        st.markdown('<div class="how-to-read"><p>'
            '📊 <strong>How to read:</strong> The gold shape is the average champion profile. '
            'Pick any team from any year to overlay their profile. '
            'Where your team <strong>exceeds the champion shape</strong>, they\'re elite in that area. '
            'Where they <strong>fall short</strong>, that\'s a potential tournament weakness.</p></div>',
            unsafe_allow_html=True)

        cc1, cc2 = st.columns([1, 2])
        with cc1:
            champ_year = st.selectbox("Season", sorted(shooting['YEAR'].unique(), reverse=True), key="ch_yr")
        ch_yr_teams = sorted(shooting[shooting['YEAR'] == champ_year]['TEAM'].unique())
        with cc2:
            champ_default = g_fav if g_fav != "—" and g_fav in ch_yr_teams else ch_yr_teams[0]
            champ_team = st.selectbox("Compare Team", ch_yr_teams,
                index=ch_yr_teams.index(champ_default) if champ_default in ch_yr_teams else 0,
                key="ch_team")

        # Champion averages (across all years)
        champ_data = _team_runs[_team_runs['deepest'] == 1]
        team_row = shooting[(shooting['YEAR'] == champ_year) & (shooting['TEAM'] == champ_team)]

        if len(team_row) == 0:
            st.warning(f"No shooting data for {champ_team} in {champ_year}.")
        elif len(champ_data) == 0:
            st.warning("No champion data available.")
        else:
            team_row = team_row.iloc[0]

            radar_metrics = ['DUNKS SHARE','CLOSE TWOS FG%','FARTHER TWOS FG%',
                             'THREES FG%','THREES SHARE','THREES FG%D','CLOSE TWOS FG%D']
            radar_names = ['Dunk Share','Close 2s FG%','Mid-Range FG%',
                           '3PT FG%','3PT Volume','3PT Defense\n(lower=better)',
                           'Close 2s Defense\n(lower=better)']

            # Normalize both to 0-100 using all tournament teams as reference
            all_ss = shooting  # use all years for percentile reference
            fig = go.Figure()

            # Champion archetype
            champ_vals = []
            champ_hover = []
            for col, label in zip(radar_metrics, radar_names):
                inv = 'lower' in label
                raw = champ_data[col].mean()
                p = pctile(raw, all_ss[col], invert=inv)
                champ_vals.append(p)
                champ_hover.append(f"{label}: {raw:.1f} ({p:.0f}th %ile)")
            champ_vals.append(champ_vals[0]); champ_hover.append(champ_hover[0])
            labels_c = radar_names + [radar_names[0]]
            fig.add_trace(go.Scatterpolar(r=champ_vals, theta=labels_c, fill='toself',
                name='Champion Avg', line=dict(color='#fbbf24', width=3),
                fillcolor='rgba(251,191,36,0.12)', hovertext=champ_hover, hoverinfo='text'))
            ...
            for col, label in zip(radar_metrics, radar_names):
                ...
                p = pctile(raw, all_ss[col], invert=inv)
                team_vals.append(p)
                team_hover.append(f"{label}: {raw:.1f} ({p:.0f}th %ile)")
            team_vals.append(team_vals[0]); team_hover.append(team_hover[0])
            fig.add_trace(go.Scatterpolar(r=team_vals, theta=labels_c, fill='toself',
                name=f'{champ_team} ({champ_year})', line=dict(color=GREEN, width=2),
                fillcolor='rgba(34,197,94,0.12)', hovertext=team_hover, hoverinfo='text'))
            ...
            # ── Gap Analysis Table ───────────────────────────────────
            st.markdown("#### Gap Analysis: Your Team vs Champions")
            gap_rows = []
            for col, label in zip(radar_metrics, radar_names):
                inv = 'lower' in label
                champ_avg = champ_data[col].mean()
                team_val = team_row[col]
                if inv:
                    diff = champ_avg - team_val  # lower is better, so positive = team is better
                else:
                    diff = team_val - champ_avg  # higher is better
                gap_rows.append({
                    'Metric': label.replace('\n', ' '),
                    f'{champ_team}': f"{team_val:.1f}",
                    'Champion Avg': f"{champ_avg:.1f}",
                    'Gap': f"{diff:+.1f}",
                    'Verdict': '✅ Above' if diff >= 0 else '⚠️ Below'
                })
            gap_df = pd.DataFrame(gap_rows)
            st.dataframe(gap_df, use_container_width=True, hide_index=True)

            above = sum(1 for r in gap_rows if '✅' in r['Verdict'])
            total = len(gap_rows)
            if above == total:
                st.success(f"**{champ_team}** meets or exceeds the champion archetype in all {total} metrics!")
            elif above >= total * 0.7:
                st.info(f"**{champ_team}** matches the champion archetype in {above}/{total} metrics — strong profile.")
            else:
                st.warning(f"**{champ_team}** matches only {above}/{total} champion metrics — some gaps to close.")


# ═══════════════════════════════════════════════════════════════════════════
#  MASTER 4: CONFERENCE INTELLIGENCE
# ═══════════════════════════════════════════════════════════════════════════
with master_conf:
    ...
    if not sc: st.info("Select at least one conference.")
        ...
        st.markdown('<div class="how-to-read"><p>'
            ...
            '<strong>Adj EM</strong> = adjusted efficiency margin (points per 100 possessions above average — higher is better). '
            '<strong>BARTHAG</strong> = probability of beating an average team. '
            '<strong>Talent</strong> = recruiting rating. '
            '<strong>Elite SOS</strong> = strength of schedule against elite opponents. '
            '<strong>WAB</strong> = wins above bubble (positive = tournament-worthy conference).'
            '</p></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="how-to-read"><p>'
            '📊 <strong>The "Four Factors"</strong> are Dean Oliver\'s key drivers of basketball success: '
            '<strong>Effective FG%</strong> (shooting efficiency including 3-pointers), '
            '<strong>Turnover%</strong> (lower is better on offense, higher is better on defense), '
            '<strong>Rebounding%</strong> (offensive = second chances, defensive = ending possessions), and '
            '<strong>Free Throw Rate</strong> (getting to the line). Green = offense, Red = defense.'
            '</p></div>', unsafe_allow_html=True)
...
with master_models:
    ...
    st.markdown('<div class="chart-guide"><p>'
        'This app uses <strong>5 logistic regression models</strong>, each adding more features. '
        'The <strong>AUC-ROC</strong> score (0.5 = random guess, 1.0 = perfect) measures how well each model '
        'distinguishes winners from losers. The metrics below show each model\'s AUC — higher is better.'
        '</p></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="how-to-read"><p>'
        '📊 <strong>Reading the ROC chart (left):</strong> Each curve shows a model\'s ability to '
        'distinguish winners from losers. The further the curve bows toward the <strong>top-left corner</strong>, '
        'the better the model. The dotted diagonal = pure random guessing. '
        'The <strong>bar chart (right)</strong> summarizes each model\'s AUC score with error bars from 5-fold cross-validation.'
        '</p></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="how-to-read"><p>'
        '📊 <strong>How to read:</strong> Each bar shows a feature\'s <strong>standardized coefficient</strong> — '
        'its influence on win probability after accounting for scale differences. '
        '<strong>Green bars</strong> (positive) increase win probability. '
        '<strong>Red bars</strong> (negative) decrease it. '
        'Longer bars = stronger effect. For example, a large positive KenPom EM bar means '
        'teams with higher efficiency margins are significantly more likely to win.'
        '</p></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="how-to-read"><p>'
        ...
        '<strong>AUC</strong> = area under the ROC curve (predictive accuracy). '
        '<strong>Std</strong> = standard deviation across 5 cross-validation folds (lower = more stable). '
        '<strong>Obs</strong> = number of games used for training. '
        'The Full model has fewer observations because KenPom data starts in 2012.'
        '</p></div>', unsafe_allow_html=True)
    ms=pd.DataFrame([
...
# ── Footer ────────────────────────────────────────────────────────────────
st.markdown("---")
st.caption("NCAA Tournament Analytics Hub · Created by Levi Schrand · Data: 2008–2025 · "
