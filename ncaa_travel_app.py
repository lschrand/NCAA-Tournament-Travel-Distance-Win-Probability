...
def load_and_process_data():
    ...
    for candidate in ['.', 'data', os.path.dirname(__file__) if '__file__' in dir() else '.']:
        if os.path.exists(os.path.join(candidate, 'Tournament_Locations.csv')):
            data_dir = candidate
            break

    locations = pd.read_csv(os.path.join(data_dir, 'Tournament_Locations.csv'))
    matchups  = pd.read_csv(os.path.join(data_dir, 'Tournament_Matchups.csv'))
...
try:
    ...
    DATA_LOADED = False
    st.error(f"**Error loading data:** {e}\n\nMake sure `Tournament_Locations.csv` and "
             f"`Tournament_Matchups.csv` are in the same directory as this script.")
    st.stop()
