...
Place the following CSV files in the same directory:
    ...
    - TeamRankings.csv
    - KenPom_Preseason.csv
    - Conference_Stats.csv
"""
...
def load_and_process_data():
    ...
    kenpom = pd.read_csv(os.path.join(data_dir, 'KenPom_Preseason.csv'))
    conf_stats = pd.read_csv(os.path.join(data_dir, 'Conference_Stats.csv'))
...
try:
    ...
    st.error(f"**Error loading data:** {e}\n\nMake sure `Tournament Locations.csv`, "
             f"`Tournament Matchups.csv`, `TeamRankings.csv`, `KenPom_Preseason.csv`, "
             f"and `Conference_Stats.csv` are in the same directory as this script.")
    st.stop()
...
tab_pred, tab_exp, tab_rnd, tab_seed, tab_team, tab_h2h, tab_conf, tab_enhanced, tab_model = st.tabs([
    "🎯 Win Predictor",
    "📊 Distance Explorer",
    "🔄 By Round",
    "🌱 By Seed",
...
# ═══════════════════════════════════════════════════════════════════════════
#  TAB 7: CONFERENCE STRENGTH IMPACT
# ═══════════════════════════════════════════════════════════════════════════
