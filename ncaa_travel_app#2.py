...
def best_model_predict(row_a, row_b):
    ...
    try:
        ...
        if all(pd.notna(v) for v in va+vb):
            ...
            return pa/(pa+pb),pb/(pa+pb),'Full model',cv5.mean()*100
    except: pass
    try:
        ...
        if all(pd.notna(v) for v in va+vb):
            ...
            return pa/(pa+pb),pb/(pa+pb),'Enhanced model',cv4.mean()*100
    except: pass
    ...
    return pa/(pa+pb),pb/(pa+pb),'Simple model',cv2.mean()*100
...
with hl:
    ...
    st.markdown('<p style="color:#94a3b8;font-size:0.85rem;margin-top:4px;">'
        'Created by <strong style="color:#f97316;">Levi Schrand</strong> · Purdue University</p>',
        unsafe_allow_html=True)
with hr:
    try:
        _p=os.path.join(os.path.dirname(__file__),'teams_pic.jpg') if '__file__' in dir() else 'teams_pic.jpg'
        if os.path.exists(_p): st.image(_p,width=120)
        elif os.path.exists('teams_pic.jpg'): st.image('teams_pic.jpg',width=120)
    except: pass
st.markdown("---")
...
with m_home:
    ...
    g3.metric("Best Model Accuracy",f"{max(cv4.mean(),cv5.mean())*100:.0f}%")
    ...
    with tab_enh:
        ...
        if mode=="Pick Tournament Teams":
            ...
            if ta==tb: st.info("Pick two different teams.")
                ...
                preds.append(("Basic",pa/(pa+pb),pb/(pa+pb),cv2.mean()))
                try:
                    va=[ra[f] for f in f4];vb=[rb[f] for f in f4]
                    if all(pd.notna(v) for v in va+vb):
                        pa=lr4.predict_proba(sc4.transform([va]))[0][1]
                        pb=lr4.predict_proba(sc4.transform([vb]))[0][1]
                        preds.append(("+ Power Ratings",pa/(pa+pb),pb/(pa+pb),cv4.mean()))
                except: pass
                try:
                    ...
                    if all(pd.notna(v) for v in va+vb):
                        pa=lr5.predict_proba(sc5.transform([va]))[0][1]
                        pb=lr5.predict_proba(sc5.transform([vb]))[0][1]
                        preds.append(("+ KenPom (best)",pa/(pa+pb),pb/(pa+pb),cv5.mean()))
                except: pass
...
with m_conf:
    ...
    if not sc2: st.info("Select at least one conference.")
        ...
        ).background_gradient(subset=['Strength'],cmap='RdYlGn').background_gradient(subset=['Wins Above Bubble'],cmap='RdYlGn'),
...
with m_models:
    ...
    m1.metric("Distance Only",f"{cv1.mean()*100:.0f}%")
    m2.metric("+ Seed",f"{cv2.mean()*100:.0f}%")
    m3.metric("+ Time Zones",f"{cv3.mean()*100:.0f}%")
    m4.metric("+ Power Ratings",f"{cv4.mean()*100:.0f}%")
    m5.metric("+ KenPom (Best)",f"{cv5.mean()*100:.0f}%")
    ...
    for Xs,md,yt,nm,cl,ds in[(X1s,lr1,ya,f'Distance ({cv1.mean()*100:.0f}%)',MUTED,'dot'),
        (X2s,lr2,ya,f'+Seed ({cv2.mean()*100:.0f}%)',ORANGE,'solid'),
        (X3s,lr3,ya,f'+TZ ({cv3.mean()*100:.0f}%)',CYAN,'dash'),
        (X4s,lr4,y4,f'+Power ({cv4.mean()*100:.0f}%)',GREEN,'solid'),
        (X5s,lr5,y5,f'+KenPom ({cv5.mean()*100:.0f}%)',PURPLE,'solid')]:
    ...
    fig.add_trace(go.Bar(x=nms,y=[a*100 for a in aucs],marker_color=bcs,
        error_y=dict(type='data',array=[s*100 for s in sds],visible=True,color=MUTED),
        text=[f"{a*100:.0f}%" for a in aucs],textposition='outside',showlegend=False),row=1,col=2)
    fig.add_hline(y=50,line_dash="dash",line_color=MUTED,row=1,col=2)
    fig.update_layout(**plotly_layout(height=420))
    fig.update_yaxes(range=[45,max(aucs)*100+8],row=1,col=2)
    st.plotly_chart(fig,use_container_width=True)
    bottom_line(f"Our best model is {max(aucs)*100:.0f}% accurate — significantly better than the {cv1.mean()*100:.0f}% "
    ...
    fig=make_subplots(1,2,subplot_titles=[f"Enhanced Model ({cv4.mean()*100:.0f}% acc.)",
        f"Full Model ({cv5.mean()*100:.0f}% acc.)"])
    ...
    with st.expander("📊 Detailed Model Summary Table"):
        ms=pd.DataFrame([
            dict(Model='1. Distance',What_It_Uses='Distance only',Accuracy=f"{cv1.mean()*100:.1f}%",Games=f"{len(ya):,}",Years='2008–2025'),
            dict(Model='2. + Seed',What_It_Uses='Distance, Seed',Accuracy=f"{cv2.mean()*100:.1f}%",Games=f"{len(ya):,}",Years='2008–2025'),
            dict(Model='3. + Time Zones',What_It_Uses='Distance, Seed, Time Zones',Accuracy=f"{cv3.mean()*100:.1f}%",Games=f"{len(ya):,}",Years='2008–2025'),
            dict(Model='4. Enhanced',What_It_Uses='Distance, Seed, Power Rating, SOS',Accuracy=f"{cv4.mean()*100:.1f}%",Games=f"{len(y4):,}",Years='2008–2025'),
            dict(Model='5. Full (Best)',What_It_Uses='All above + KenPom, Improvement, Top-25 Wins',Accuracy=f"{cv5.mean()*100:.1f}%",Games=f"{len(y5):,}",Years='2012–2025')])
        st.dataframe(ms,use_container_width=True,hide_index=True)

# ── Footer ────────────────────────────────────────────────────────────────
st.markdown("---")
st.caption("NCAA Tournament Analytics Hub · Created by Levi Schrand · Purdue University · "
